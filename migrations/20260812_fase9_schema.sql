...
CREATE TABLE IF NOT EXISTS public.extensoes (
    ...
    sigla text NOT NULL, -- Usada no prefixo da licença (ex: LOV, ZAP)
    icone_url text,
    capa_url text,
    descricao text,
    ...
    versao_atual text,
    screenshots text[], -- URLs das imagens
...
CREATE POLICY "Qualquer um pode ver extensoes ativas" ON public.extensoes FOR SELECT USING (status = 'ativo');
CREATE POLICY "Admins possuem controle total sobre extensoes" ON public.extensoes ALL TO authenticated USING (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Clientes e anonimos podem ver versoes" ON public.product_versions FOR SELECT USING (true);
CREATE POLICY "Admins podem gerenciar versoes" ON public.product_versions ALL TO authenticated USING (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Publico pode ver configuracao de trial" ON public.extensao_trials FOR SELECT USING (true);
CREATE POLICY "Admins podem gerenciar configuracao de trial" ON public.extensao_trials ALL TO authenticated USING (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Usuario vê seus proprios trials" ON public.trials_emitidos FOR SELECT TO authenticated USING (auth.uid() = auth_user_id);
CREATE POLICY "Trial pode ser emitido por usuario autenticado" ON public.trials_emitidos FOR INSERT TO authenticated WITH CHECK (auth.uid() = auth_user_id);
...
CREATE POLICY "Usuario vê seus resets" ON public.hwid_resets FOR SELECT TO authenticated USING (auth.uid() = auth_user_id);
