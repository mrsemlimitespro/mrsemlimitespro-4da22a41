...
const { Route } = await import('@/routes/api/public/ext/validate-license');
...
describe('validate-license handler', () => {
  beforeEach(() => {
  ...
  it('devolve os aliases de sessão esperados pela extensão sem remover o contrato V17', async () => {
    const handler = Route.options.server?.handlers?.POST as (args: { request: Request }) => Promise<Response>;
    const response = await handler({
      request: new Request('https://mr.example/api/public/ext/validate-license', {
        method: 'POST',
        headers: {
